...
import { Plus, SlidersHorizontal, Search, CheckSquare, Square as SquareIcon, ArrowRight, Shield } from "lucide-react";
    ...
    <AppShell>
      <div className="p-8 max-w-7xl w-full mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-text tracking-tight">Schedules</h1>
            <p className="text-xs text-[#888892] mt-1">
              Customer Order Supervision Status and Details
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              className="bg-[#141418] border-[#1c1c24] text-text-muted hover:text-text"
            >
              Export
            </Button>
            <Button
        ...
        <div className="flex items-center justify-between gap-4 pt-2">
          ...
          <div className="relative w-64">
            ...
            <input
              ...
              placeholder="Search..."
              className="h-8 w-full rounded-lg border border-[#1c1c24] bg-[#141418] pl-9 pr-3 text-xs text-text placeholder:text-[#656570] focus:border-[#2a2a34] focus:outline-none"
            />
          </div>
        </div>
                  ...
                  <tr className="border-b border-[#1c1c24] text-[#656570] font-medium text-[11px]">
                    <th className="py-3 pl-2 pr-2 w-8">
                      <button
                        ...
                        className="text-[#656570] hover:text-text"
                      >
                ...
                <tbody className="divide-y divide-[#1c1c24]">
                  {filteredRuns.map((run) => {
                    ...
                    return (
                      <tr
                        key={run.id}
                        className={`transition-colors hover:bg-[#15151c] ${
                          isSelected ? "bg-[#161622]" : ""
                        }`}
                      >
                        {/* Checkbox */}
                        ...
                        <td className="py-3.5 px-3">
                          <div className="flex items-center gap-2.5">
                            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#1e1e28] text-text font-bold text-[10px]">
                              G
                            </div>
                        ...
                        {/* Events / Actions */}
                        <td className="py-3.5 px-3 text-[#888892]">
                          <span>{state.event_count ?? 0} Events</span>
                          <span className="text-[#444450] mx-1.5">•</span>
                          <span>{state.action_count ?? 0} Actions</span>
                        </td>
